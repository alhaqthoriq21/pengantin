...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <div class="card" id="dataFotoHeader">
                ...
                <div class="card-body table-responsive">
                    <table class="table table-hover table-bordered table-outline mb-0" id="dataFotoHeader">
                        ...
                        <tbody>
                            <?php $no = 1; ?>
                            @foreach($fotoHeader as $data)
                            <tr>
                                <th scope="row">{{ $no }}</th>
                                <td>{{ $data->calon->slug }}</td>
                                <td width="200px"> <a href="{{route('update.status.fotoHeader', $data->id)}}"
                                ...
                                <td><a href="{{route('fotoHeader.detail', $data->id)}}"
                    ...
                    <div class="pagination pagination-sm m-0 float-right mt-2">
                        {{$fotoHeader->links("vendor.pagination.bootstrap-4")}}
                    </div>
                </div>
            </div>
            ...
            <div class="modal" tabindex="-1" role="dialog" id="modalUpdateStatus">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{route('fotoHeader.updateStatus')}}" id="formUpdateStatus" method="POST">
                            <div class="modal-body">
                                @csrf
                                <input type="hidden" name="fotoHeaderId" id="fotoHeaderId" value="0">
                                <div class="row">
                                    <div class="col-md-6">
                                        <label for="">Status</label>
                                        <select name="status" id="status" class="form-control">
                                            @foreach ($statusFotoHeader as $key => $value)
            ...
            <div class="card" id="tambahFotoHeader" style="display: none">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h5 class="h3 mb-0">Form Foto Header</h5>
                        </div>
                    </div>
                </div>
                <div class="card-body table-responsive">
                    <form action="{{route('fotoHeader.save')}}" method="post" id="formFotoHeader"
...
$(document).ready(function() {
    $("tbody").on("click", ".btn-status", function(e) {
        e.preventDefault();
        const fotoHeaderId = $(this).attr("data-value");
        const status = $(this).attr("data-status");
        $('#fotoHeaderId').val(fotoHeaderId);
        $('#status').val(status);
    ...
    const dataFotoHeader = $('#dataFotoHeader');
    const tambahFotoHeader = $('#tambahFotoHeader');
    const formFotoHeader = $('#formFotoHeader');


    btnAdd.click(function(e) {
        e.preventDefault();
        formFotoHeader.trigger("reset");
        showForm();
        $('#statusSave').val(0);
    })
    ...
    function showForm() {
        dataFotoHeader.hide();
        tambahFotoHeader.show();
    }

    function hideForm() {
        dataFotoHeader.show();
        tambahFotoHeader.hide();
    }
})
</script>
@endpush